...
class Story {
  ...
  getRecommendations() {
    this.music = false
    if (this.music) {
        console.log(`Music I see!`);
        this.selection = '139998151'  // Blues
    } else{
        console.log(`News I see!`);
        this.selection = 'npr'
        // this.selection = '139998151'  // Blues
    }
    return this.npr.one.listening.getRecommendations({channel: this.selection})

    // return this.npr.one.listening.getAggRecommendations({ aggId: this.selection })
    // if (!this.music) return this.npr.one.listening.getAggRecommendations({ aggId: '122356178' })  // songs we love

    // return this.npr.one.listening.getAggRecommendations({ aggId: '1004' })  // World news
    // return this.npr.one.listening.getAggRecommendations({ aggId: '1019' })  // Technology
    // return this.npr.one.listening.getAggRecommendations({ aggId: '64' })  // Invisibilia
    // return this.npr.one.listening.getAggRecommendations({ aggId: '510324' })  // Rough Translation
    // return this.npr.one.listening.getAggRecommendations({ aggId: '122356178' })  // songs we love
    // return this.npr.one.listening.getAggRecommendations({ aggId: '347139849' })  // jazz night in america
    // return this.npr.one.listening.getAggRecommendations({ aggId: '15396553' })  // led zeppelin
    // return this.npr.one.listening.getAggRecommendations({ aggId: '114219171' })  // the xx
    // return this.npr.one.listening.getAggRecommendations({ aggId: '455636294' })  // jonny greenwood
    // return this.npr.one.listening.getAggRecommendations({ aggId: '381443519' })  // Kojo Nnamdi
    .then((rec) => {
  ...
  markInteresting(sec) {
    ...
    rating.rating = 'THUMBUP';  // Removed S
    rating.elapsed = sec;
    rating.timestamp = (new Date()).toISOString();
    this.ratings.push(rating);
